...
def welcome(update: Update, context: CallbackContext):
    if update.message.chat.id in _ADMIN:  # !!! To check if the user is admin or not
        logger.info(update.message.chat.first_name +" (admin) started the bot")
        ...
        logger.info(update.message.chat.first_name+ " (user) started the bot")
        _USERS = LOGGINGCOLLECTION.find_one()['user']
...
def messageHandler(update: Update, context: CallbackContext):
    if context.user_data.get("now", "") == "search":
        if update.message.text in _CODE:
            context.bot.send_document(
                chat_id=update.effective_chat.id, document=_CODE[update.message.text])
        else:
            context.bot.send_message(chat_id = update.effective_chat.id, text = "Sorry I don't have that for now")
...
def adminHandler(update: Update, context: CallbackContext):
    ...
    if text == 'activity':
        ...
        if tempD:
            ...
            for user in tempD:
                text+="<b>"+str(
                    tempD[user]['firstName']) +"</b>"+ " has started the bot at " + str(tempD[user]['currentTime'])
                text +='\n'
            context.bot.send_message(chat_id = update.effective_chat.id , text = text, parse_mode = "HTML")
    elif text == 'anounce':
        logger.info("Admin sent anouncemnt to all users")
        context.bot.send_message(
...
def moreHandler(update: Update, context: CallbackContext):
    ...
    if text == 'more':
        try:
            ...
            logger.info("Index is Out of range")
